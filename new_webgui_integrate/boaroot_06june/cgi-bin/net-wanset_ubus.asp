...
elseif Request_Form("Wan_Flag") = "6" then
	...
	TCWebApi_set("WLan_Common","elinksync","SyncWlCFG")
	TCWebApi_set("WLan11ac_Common", "elinksync", "SyncWlCFG")
	TCWebApi_commit("Wlan_Common")
	TCWebApi_commit("Wlan11ac_Common")
	TCWebApi_commit("apcli_entry0")
	TCWebApi_save()
...
elseif Request_Form("Wan_Flag") = "10" then
	 if tcwebApi_get("WebCustom_Entry", "isUBUSSupported", "h") = "Yes" then
	 	 TCWebApi_set("Wan_Common", "WorkMode", "workMode")
	 	 TCWebApi_set("APCli_Common", "currentRadio", "transModeValue")
		 TCwebApi_commit("Wan_Common")
		 TCWebApi_save()
	 end if
elseif Request_Form("Wan_Flag") = "11" then
	 TCWebApi_set("WLan_Common", "elinksync", "SyncWlCFG")
	 TCWebApi_set("WLan11ac_Common", "elinksync", "SyncWlCFG")
	 TCWebApi_commit("Wlan_Common")
	 TCWebApi_commit("Wlan11ac_Common")
	 TCWebApi_save()
elseif Request_Form("Wan_Flag") = "12" then
	 TCWebApi_set("WLan_Common", "elinksync", "SyncWlCFG")
	 TCWebApi_set("WLan11ac_Common", "elinksync", "SyncWlCFG")
	 TCWebApi_commit("Wlan_Common")
	 TCWebApi_commit("Wlan11ac_Common")
	 TCWebApi_save()
elseif Request_Form("Wan_Flag") = "13" then
	 TCWebApi_set("WLan_Common", "elinksync", "SyncWlCFG")
	 TCWebApi_set("WLan11ac_Common", "elinksync", "SyncWlCFG")
	 TCWebApi_commit("Wlan_Common")
	 TCWebApi_commit("Wlan11ac_Common")
...
<% end if %>
      ...
      <TABLE height="100%" cellSpacing=0 cellPadding=0 border=0>
        ...
        <TR>
          ...
          <TD width=494>　</TD>
          <TD vAlign=top class=cus_help background=/img/panel4.gif rowSpan=10>
...
<% if tcwebApi_get("WebCustom_Entry","isUBUSSupported","h") = "Yes" then %>	
	      	...
	      	<TBODY>
	        	<TR>
	        		<TD width=10>&nbsp;</TD>
	        		<TD><P>提示：切换AP工作模式时，会改变相应的传输模式，并且需要重新配置wan连接参数。<P></TD>
	       		</TR>
	      	</TBODY>
	      </TABLE>-->
        	...
        	<TR>
        		<TD width=10>&nbsp;</TD>
          	<TD width=150><%tcWebApi_get("String_Entry","APWORKMODE","s")%></TD>
          	<TD>
          		<LABEL>
          			<SELECT id=workMode onchange=changeWorkMode() name=workMode> 
          				<OPTION value="router" <%if tcWebApi_get("Wan_Common","WorkMode","h") = "router" then asp_Write("selected") end if%>>router</OPTION>
          				<OPTION value="bridge" <%if tcWebApi_get("Wan_Common","WorkMode","h") = "bridge" then asp_Write("selected") end if%>>bridge</OPTION>
          				<OPTION value="repeater" <%if tcWebApi_get("Wan_Common","WorkMode","h") = "repeater" then asp_Write("selected") end if%>>repeater</OPTION>
          			</SELECT> 
        			</LABEL>
        		</TD>
        		<TD><INPUT id=btnChangeMode onclick="btnChangeWorkMode()" name=btnChangeMode value="<%tcWebApi_get("String_Entry","SwitchText","s")%>" type=button ></TD> 
        	</TR>

        	<TR id="tr_transmode" style='display:none'>
        		<TD width=10>&nbsp;</TD>
          	<TD width=150><%tcWebApi_get("String_Entry","TransferModeText","s")%>&nbsp;</TD>
          	<TD>
							<INPUT id=transMode type=radio value="0" name="transMode" <%if tcWebApi_get("APCli_Common","currentRadio","h") = "0" then  asp_Write("checked") end if%>>
              APClient-2.4G&nbsp;&nbsp; 
							<INPUT id=transMode type=radio value="1" name="transMode" <%if tcWebApi_get("APCli_Common","currentRadio","h") = "1" then asp_Write("checked") end if%>>
              APClient-5G&nbsp;&nbsp; 
              <input type="hidden" name="transModeValue" value="0">
         	</TD>
         </TR>

         	<TR id= "tr_elink_sync" name="elink_sync">
			<TD width=10>&nbsp;</TD>
			<TD colspan="2">
				<INPUT onclick=SyncWlanCFG() type=checkbox name=cb_SyncWlCFG><%tcWebApi_get("String_Entry","APWlanSetting","s")%>
				<INPUT id=SyncWlCFG type=hidden name=SyncWlCFG value="<%if tcWebApi_get("WLan_Common","elinksync","h") <> "N/A" then tcWebApi_get("WLan_Common","elinksync","s") else asp_write("on") end if%>">
			</TD>
			<TD><INPUT id=btnSave1 onclick=SaveElinkSyncCFG() name=btnSave1 value="<%tcWebApi_get("String_Entry","BtnSaveText","s")%>" type=button></TD> 
		</TR>
				</TBODY>
				</TABLE>
			
            <TABLE id="EtherTable" name="EtherTable" cellSpacing=0 cellPadding=0 width="100%" border=0 >
              <TBODY>
              <TR>
                        <TD width=10>&nbsp;</TD>
                <TD>
...
<% if tcwebApi_get("WebCustom_Entry","isNPUSupported","h") <> "Yes" then %>	
									<OPTION value="PON" <%if tcWebApi_get("Wan_Common","TransMode","h") <> "Ethernet" then asp_Write("selected") end if%>>PON
<% end if %>
								</SELECT>
							</LABEL>
						</TD>
...
<% if tcWebApi_get("WebCustom_Entry", "isMultiUserITFSupported","h") = "Yes" then %>
		aCurBindFlag[8] = "<% tcWebApi_get("WanInfo_WanIF","SSID5","s") %>";
		aCurBindFlag[9] = "<% tcWebApi_get("WanInfo_WanIF","SSID6","s") %>";
		aCurBindFlag[10] = "<% tcWebApi_get("WanInfo_WanIF","SSID7","s") %>";
		aCurBindFlag[11] = "<% tcWebApi_get("WanInfo_WanIF","SSID8","s") %>";
<% end if %>
<% if tcWebApi_get("WebCustom_Entry", "isWLanACSupported","h") = "Yes" then %>
<% if tcWebApi_get("WebCustom_Entry", "isMultiUserITFSupported","h") <> "Yes" then %>
		...
		for(k=0; k<12; k++)
<% else %>
		aCurBindFlag[12] = "<% tcWebApi_get("WanInfo_WanIF","SSID1AC","s") %>";
		aCurBindFlag[13] = "<% tcWebApi_get("WanInfo_WanIF","SSID2AC","s") %>";
		aCurBindFlag[14] = "<% tcWebApi_get("WanInfo_WanIF","SSID3AC","s") %>";
		aCurBindFlag[15] = "<% tcWebApi_get("WanInfo_WanIF","SSID4AC","s") %>";
		aCurBindFlag[16] = "<% tcWebApi_get("WanInfo_WanIF","SSID5AC","s") %>";
		aCurBindFlag[17] = "<% tcWebApi_get("WanInfo_WanIF","SSID6AC","s") %>";
		aCurBindFlag[18] = "<% tcWebApi_get("WanInfo_WanIF","SSID7AC","s") %>";
		aCurBindFlag[19] = "<% tcWebApi_get("WanInfo_WanIF","SSID8AC","s") %>";
		for(k=0; k<20; k++)
<% end if %>
<% else %>
<% if tcWebApi_get("WebCustom_Entry", "isMultiUserITFSupported","h") = "Yes" then %>
...
<% end if %>
		{
			...
			if((aTemp1[i] == "Yes") && (aTemp2[i] == "Yes") && (aTemp3[i] == "No"))
			{
<%if TCWebApi_get("WebCustom_Entry","isDSLiteSupported","h" ) = "Yes" then%>
...
<% if tcWebApi_get("WebCustom_Entry", "isMultiUserITFSupported","h") <> "Yes" then %>	
					if ( ("IPv4" == strIpversion && vForm.curSetIndex.value == parseInt(aTemp1[42+2*i]))
						...
						continue;
<% else %>
					if ( ("IPv4" == strIpversion && vForm.curSetIndex.value == parseInt(aTemp1[66+2*i]))
						 || ("IPv6" == strIpversion && vForm.curSetIndex.value == parseInt(aTemp1[66+2*i+1]))
						 || ("IPv4/IPv6" == strIpversion && v4v6BindCheck(vForm.curSetIndex.value, parseInt(aTemp1[66+2*i]), parseInt(aTemp1[66+2*i+1]))) )
						continue;
<% end if %>
<% else %>
<% if tcWebApi_get("WebCustom_Entry", "isMultiUserITFSupported","h") <> "Yes" then %>	
					if ( ("IPv4" == strIpversion && vForm.curSetIndex.value == parseInt(aTemp1[24+2*i]))
						...
						continue;
<% else %>
					if ( ("IPv4" == strIpversion && vForm.curSetIndex.value == parseInt(aTemp1[36+2*i]))
						 || ("IPv6" == strIpversion && vForm.curSetIndex.value == parseInt(aTemp1[36+2*i+1]))
						 || ("IPv4/IPv6" == strIpversion && v4v6BindCheck(vForm.curSetIndex.value, parseInt(aTemp1[36+2*i]), parseInt(aTemp1[36+2*i+1]))) )
						continue;
<% end if %>
<% end if %>
				}
...
<% else %>
								if(0 == aTemp1[22+2*i+1])//if ipv6 is not binded, can ok
<% end if %>
<% else %>	
<% if tcWebApi_get("WebCustom_Entry", "isMultiUserITFSupported","h") <> "Yes" then %>
								if(0 == aTemp1[8+2*i+1])//if ipv6 is not binded, can ok
<% else %>
								if(0 == aTemp1[12+2*i+1])//if ipv6 is not binded, can ok
<% end if %>
<% end if %>
									continue;
							}
...
<% else %>
								if(0 == aTemp1[22+2*i])//if ipv4 is not binded, can ok
<% end if %>
<% else %>	
<% if tcWebApi_get("WebCustom_Entry", "isMultiUserITFSupported","h") <> "Yes" then %>
								if(0 == aTemp1[8+2*i])//if ipv4 is not binded, can ok
<% else %>
								if(0 == aTemp1[12+2*i])//if ipv4 is not binded, can ok
<% end if %>
<% end if %>
									continue;
							}
...
<% else %>
				if((("IPv4" == strIpversion) && (0 == aTemp1[22+2*i])) || (("IPv6" == strIpversion) && (0 == aTemp1[22+2*i+1]))){	
<% end if %>
					continue;
				}
<% else %>
...
<% else %>
				if((("IPv4" == strIpversion) && (0 == aTemp1[12+2*i])) || (("IPv6" == strIpversion) && (0 == aTemp1[12+2*i+1]))){
<% end if %>
					continue;
				}
<% end if %>					
...
/*
	...
	{
		alert('SSID1 "' + getValue('wlanssidap') + '" 应该是以"CMCC-"开头的字符串');
		return false;
	}
*/
...
/*	
	...
	if ( the_substring != 'CMCC-' )
	{
		alert('SSID1 "' + getValue('wlanACssidap') + '" 应该是以"CMCC-"开头的字符串');
		return false;
	}
*/
...
function btnChangeWorkMode()
{
	var	vForm = document.ConfigForm;
	var oldWorkMode = '<% tcWebApi_get("Wan_Common","WorkMode","s") %>';
	if(oldWorkMode == vForm.workMode.value)
	{
		if ( vForm.workMode.value == 'repeater' )
		{
			var oldTransMode = '<% tcWebApi_get("APCli_Common","currentRadio","s") %>';
			if ( oldTransMode == getRadioVal("transMode") )
			{
				alert("未进行APClient传输模式切换!");
				return;
			}
		}
		else
	...
	var meshrole = '<% tcWebApi_get("Mesh_common","DeviceRole","s") %>';
	if ( vForm.workMode.value == 'router' )
	{
		if ( meshenable == '1' && meshrole == '2' )
		{
			alert("当前mesh开启且为agent，AP不能切换到route工作模式");
			return;
		}
	}
	else if ( vForm.workMode.value == 'repeater' )
	{
		if ( meshenable == '1' )
		{
			alert("当前mesh开启，AP不能切换到repeater工作模式");
			return;
		}
		
		vForm.transModeValue.value = getRadioVal("transMode");
		if ( vForm.transModeValue.value == null )
		{
			alert("请先选择传输模式");
			return;
		}
	...
	if ( confirm("您是否确认要切换传输模式？") == false )
		return;
	
	vForm.Wan_Flag.value = "10";

	setDisable('btnRemoveCnt',1);
	setDisable('btnAddCnt',1);
	setDisable('btnSave0',1);
	setDisable('btnCancel0',1);
	setDisable('btnSave1',1);
	setDisable('btnCancel1',1);
	setDisable('btnChangeMode',1);
	setDisplay('tr_elink_sync', 1);
	setDisplay('tr_btnSave1', 1);
	setDisplay('tr_btnSave2', 1);
...
function changeWorkMode()
{
	...
	if ( vForm.workMode.value == 'repeater' )
	{
		setDisplay('tr_transmode', 1);
		if ( oldTransMode != 'N/A' )
			setRadio('transMode', oldTransMode);
		else
			setRadio('transMode', '0');
	}
	else
	{
		setDisplay('tr_transmode', 0);
	}
	
	setDisplay('tr_elink_sync', 0);
	setDisplay('tr_btnSave1', 0);
	setDisplay('tr_btnSave2', 0);
}
<% end if %>
...
function btnRemoveWanCnt()
{
	...
	<%else%>
	if(confirm("<%tcWebApi_get("String_Entry","DeleteWANConnection2Text","s")%>"))
	<%end if%>
		return;
...
<% if TCWebApi_get("WebCustom_Entry","isMultiUserITFSupported","h" ) = "Yes" then %>
            ...
            <%end if%>
                        <DIV id=secWireless5>
						<INPUT id=cb_bindwireless5  type=checkbox name=cb_bindwireless5 <%if tcWebApi_get("WanInfo_WanIF","SSID5","h") = "Yes" then asp_Write("checked") end if%>> SSID5
            ...
            <%end if%>
                        <DIV id=secWireless6>
						<INPUT id=cb_bindwireless6 type=checkbox name=cb_bindwireless6 <%if tcWebApi_get("WanInfo_WanIF","SSID6","h") = "Yes" then asp_Write("checked") end if%>> SSID6 <INPUT id=bindwireless6 
            ...
            <%end if%>	
                        <DIV id=secWireless7>
						<INPUT id=cb_bindwireless7 type=checkbox name=cb_bindwireless7 <%if tcWebApi_get("WanInfo_WanIF","SSID7","h") = "Yes" then asp_Write("checked") end if%>> SSID7 
             ...
             <%else%>
                      <TD>
             <%end if%>
                        <DIV id=secWireless8><INPUT id=cb_bindwireless8 type=checkbox name=cb_bindwireless8 <%if tcWebApi_get("WanInfo_WanIF","SSID8","h") = "Yes" then asp_Write("checked") end if%>> SSID8 
...
<% if TCWebApi_get("WebCustom_Entry","isMultiUserITFSupported","h" ) = "Yes" then %>
				...
				<DIV id=secWirelessac5>
					<INPUT id=cb_bindwirelessac5 type=checkbox name=cb_bindwirelessac5 <%if tcWebApi_get("WanInfo_WanIF","SSIDAC5","h") = "Yes" then asp_Write("checked") end if%>> SSIDAC5
					<INPUT id=bindwirelessac5 type=hidden value=0 name=bindwirelessac5> 
				</DIV></TD>
				<TD width="75">
				<DIV id=secWirelessac6>
					<INPUT id=cb_bindwirelessac6 type=checkbox name=cb_bindwirelessac6 <%if tcWebApi_get("WanInfo_WanIF","SSIDAC6","h") = "Yes" then asp_Write("checked") end if%>> SSIDAC6
					<INPUT id=bindwirelessac6 type=hidden value=0 name=bindwirelessac6> 
				</DIV></TD>
				<TD width="77">
				<DIV id=secWirelessac7>
					<INPUT id=cb_bindwirelessac7 type=checkbox name=cb_bindwirelessac7 <%if tcWebApi_get("WanInfo_WanIF","SSIDAC7","h") = "Yes" then asp_Write("checked") end if%>> SSIDAC7 
					<INPUT id=bindwirelessac7 type=hidden value=0 name=bindwirelessac7> 
				</DIV></TD>
				<TD width="79">
				<DIV id=secWirelessac8>
					<INPUT id=cb_bindwirelessac8 type=checkbox name=cb_bindwirelessac8 <%if tcWebApi_get("WanInfo_WanIF","SSIDAC8","h") = "Yes" then asp_Write("checked") end if%>> SSIDAC8
...
<%if TCWebApi_get("WebCustom_Entry","isPPPoEProxySupported","h" ) = "Yes" then%>
					             <TR id='ppp_proxy_bi'>
				                      ...
				                      <TD>
				                            <INPUT id='ppp_proxy_bi_mode' onclick='ppp_proxy_bi_Change()' type=radio value="0" name="ppp_proxy_bi_mode" <%if tcWebApi_get("WanInfo_WanIF","pppProxyBiMode", "h") = "0" then asp_Write("checked") elseif tcWebApi_get("WanInfo_WanIF","pppProxyBiMode", "h") = "N/A" then asp_Write("checked") end if%>>不启用PPPoE代理或PPPoE路由桥混合<br>
				                            <INPUT id='ppp_proxy_bi_mode' onclick='ppp_proxy_bi_Change()' type=radio value="1" name="ppp_proxy_bi_mode" <%if tcWebApi_get("WanInfo_WanIF","pppProxyBiMode","h") = "1" then asp_Write("checked") end if%>>启用PPPoE代理<br>
				                            <INPUT id='ppp_proxy_bi_mode' onclick='ppp_proxy_bi_Change()' type=radio value="2" name="ppp_proxy_bi_mode" <%if tcWebApi_get("WanInfo_WanIF","pppProxyBiMode","h") = "2" then asp_Write("checked") end if%>>启用PPPoE路由桥混合模式<br>
...
<%end if%>
                          ...
                          <LABEL></LABEL><BR>
				                  <LEFT>
				                  	<P align=center>
					                		<INPUT id=btnSave0 onclick=btnSave() name=btnSave0 value=确定 type=button>
					                		<INPUT id=btnCancel0 onclick=onCancel() name=btnCancel0 value=取消 type=button>
				                		</P>
				                  </LEFT>
                          </TD>
                      </TR>
...
function SyncWlanCFG()
{
	...
	if (getCheckVal('cb_SyncWlCFG') == 1)
		vForm.SyncWlCFG.value = "on";
	else
		vForm.SyncWlCFG.value = "off";
}

function SaveElinkSyncCFG()
{
	var vForm = document.ConfigForm;
	var vworkmode = '<% tcWebApi_get("Wan_Common","WorkMode","s") %>';
	var transmode = '<% tcWebApi_get("APCli_Common","currentRadio","s") %>';
	
	if ( vworkmode == 'repeater' )
	{
		if ( transmode == '1' )
		{
			vForm.Wan_Flag.value = "12"; /* 5g */
		}
		else
		{
			vForm.Wan_Flag.value = "11"; /* 24g */
		}	
	}
	else
	{
		vForm.Wan_Flag.value = "13"; /* 24g + 5g */
	}
...
function SyncWlanACCFG()
{
	...
	if (getCheckVal('cb_SyncWlACCFG') == 1)
		vForm.SyncWlACCFG.value = "on";
	else
		vForm.SyncWlACCFG.value = "off";
}
...
function LockAPNode()
{
	...
	setDisplay('tr_transmode', 0);
	if ( vworkmode == 'bridge' || vworkmode == 'repeater' )
	{
		if ( vworkmode == 'repeater' )
		{
			setDisplay('tr_transmode', 1);
			if ( transmode == '1' )
			{
				setDisplay('AP5GTitle', 1);
				setDisplay('AP5GConText', 1);
				elinksync = '<% tcWebApi_get("WLan11ac_Common","elinksync","s") %>';
			}
			else
			{
				setDisplay('AP24GTitle', 1);
				setDisplay('AP24GConText', 1);
				elinksync = '<% tcWebApi_get("WLan_Common","elinksync","s") %>';
			}
		}
		
		setDisplay('EtherTable', 0);
	}
	...
	if ( elinksync == 'on' )
		setCheck('cb_SyncWlCFG',1);
	else
		setCheck('cb_SyncWlCFG',0);

	return;
}
</script>
			<TR name="AP24GTitle" id="AP24GTitle" style='display:none'>
				<TD width=157 bgColor=#ef8218 height=30  >
				<P class=Item_L1><%tcWebApi_get("String_Entry","AP24GSettingTitle","s")%></P></TD>
				<TD width=7 bgColor=#ef8218>　</TD>         
				<TD width=474>　</TD>
			
			</TR>

			<TR name="AP24GConText" id="AP24GConText" style='display:none'>
				<TD vAlign=top width=157 bgColor=#e7e7e7 height=30 >
				<P class=Item_L2></P></TD>
				<TD width=7 background=/img/panel3.gif>　</TD>
				<TD>
				<TABLE cellSpacing=10 cellPadding=0 width="100%" height=30 border=0>
				<TBODY>
				<TR>
      		<TD colSpan=2>
      			<INPUT onclick=ManualSet() name=cb_MSEnbl <%if tcWebApi_get("apcli_entry0","Enable","h") = "1" then asp_Write("checked") end if%> type=checkbox>使能2.4G AP配置
      	...
      	<TR>
      		<TD colspan="2">提示：请先在宽带配置页面选择传输模式为APClient-2.4G模式。</TD>
     		</TR>
				<TR>
					<TD>中继上行WLAN SSID：</TD>
					<TD>
						<LABEL> 
							<LABEL>
							<INPUT id=wlanssidap class='input_s1' maxLength=63 size=15 name=wlanssidap value="<%if tcWebApi_get("apcli_entry0","SSID","h") <> "N/A" then tcWebApi_get("apcli_entry0","SSID","s") else asp_write("CMCC-QLINK") end if%>"> 
							</LABEL>
						</LABEL>
					</TD>
				</TR>
				<TR>
					<TD><%tcWebApi_get("String_Entry","EncryptionTypeText","s")%></TD>
					<TD>
						<LABEL> 
							<SELECT id=wlSecurMode onchange=authModeChange() name="wlSecurMode">
								<OPTION value="OPEN" <%if TCWebApi_get("apcli_entry0","AuthMode","h" ) = "OPEN" then asp_Write("selected") end if %>>OPEN</OPTION>
								<OPTION value="WEP_OPEN" <%if TCWebApi_get("apcli_entry0","AuthMode","h" ) = "WEP_OPEN" then asp_Write("selected") end if %>>WEP_OPEN</OPTION>
								<OPTION value="WEP_SHARED" <%if TCWebApi_get("apcli_entry0","AuthMode","h" ) = "WEP_SHARED" then asp_Write("selected") end if %>>WEP_SHARED</OPTION>
								<OPTION value="WPAPSK" <%if TCWebApi_get("apcli_entry0","AuthMode","h" ) = "WPAPSK" then asp_Write("selected") end if %>>WPA-PSK</OPTION>
								<OPTION value="WPA2PSK" <%if TCWebApi_get("apcli_entry0","AuthMode","h" ) = "WPA2PSK" then asp_Write("selected") end if %>>WPA2-PSK</OPTION>
							</SELECT>
						</LABEL>
					</TD>
				</TR>
				<TR id='wpapsw'>
					<TD class='w-130'><p>WPA <%tcWebApi_get("String_Entry","PresharedKeyText","s")%></p></TD>
					<TD>
						<span id="inpsw">
						<INPUT style="FONT-FAMILY: '宋体'; width: 140px" type=password maxLength=65 id=wlWpaPsk name=wlWpaPsk value='<% if tcWebApi_get("apcli_entry0", "WPAPSK", "h") <> "N/A" then tcWebApi_get("apcli_entry0", "WPAPSK", "s") end if %>'>
						</span>
					</TD>
				</TR>
				<TR id='wlWpaD'>
					<TD class='w-130'><p>WPA <%tcWebApi_get("String_Entry","EncriptionText","s")%></p></TD>
					<TD>
						<LABEL> 
							<SELECT id=wlWpa name="wlWpa">
								<OPTION value="TKIP" <%if TCWebApi_get("apcli_entry0","EncrypType","h" ) = "TKIP" then asp_Write("selected") end if %>>TKIP</OPTION>
								<OPTION value="AES" <%if TCWebApi_get("apcli_entry0","EncrypType","h" ) = "AES" then asp_Write("selected") end if %>>AES</OPTION>
							</SELECT>
						</LABEL>
					</TD>
				</TR>
				<TR id='wlKeyBitD'>
					<TD class='w-130'><p><%tcWebApi_get("String_Entry","EncriptionLengthText","s")%></p></TD>
					<TD>
						<LABEL> 
							<SELECT id=wlKeyBit name="wlKeyBit">
								<OPTION value="128" <%if TCWebApi_get("apcli_entry0","KeyBit","h" ) = "128" then asp_Write("selected") end if %>>128-bit</OPTION>
								<OPTION value="64" <%if TCWebApi_get("apcli_entry0","KeyBit","h" ) = "64" then asp_Write("selected") end if %>>64-bit</OPTION>
							</SELECT>
						</LABEL>
					</TD>
				</TR>
				<TR id='wlKeyIdx'>
					<TD class='w-130'><p><%tcWebApi_get("String_Entry","CurentKeyIndexText","s")%></p></TD>
					<TD>
						<LABEL> 
							<SELECT id=wlKeyIndex name="wlKeyIndex">
								<OPTION value="1" <%if TCWebApi_get("apcli_entry0","DefaultKeyID","h" ) = "1" then asp_Write("selected") end if %>>1</OPTION>
								<OPTION value="2" <%if TCWebApi_get("apcli_entry0","DefaultKeyID","h" ) = "2" then asp_Write("selected") end if %>>2</OPTION>
								<OPTION value="3" <%if TCWebApi_get("apcli_entry0","DefaultKeyID","h" ) = "3" then asp_Write("selected") end if %>>3</OPTION>
								<OPTION value="4" <%if TCWebApi_get("apcli_entry0","DefaultKeyID","h" ) = "4" then asp_Write("selected") end if %>>4</OPTION>
							</SELECT>
						</LABEL>
					</TD>
				</TR>
				<TR id='wlKey'>
					<TD class='w-130'><%tcWebApi_get("String_Entry","KeyText","s")%>	</TD>
					<TD><INPUT style="width: 140px" maxLength=26 name=wlKeys value="<% if tcWebApi_get("apcli_entry0", "Key", "h") <> "N/A" then tcWebApi_get("apcli_entry0", "Key", "s") end if %>"></TD>
				</TR>
				<TR id= "tr_btnSave1" name="tr_btnSave1">
	        <TD colSpan=2>
	        	<P align=center>
							<INPUT id=btnSave1 onclick=checkAPMode(0) name=btnSave1 value="<%tcWebApi_get("String_Entry","BtnSaveText","s")%>" type=button>
							<INPUT id=btnCancel1 onclick=onCancel() name=btnCancel1 value="<%tcWebApi_get("String_Entry","BtnCancelText","s")%>" type=button>
	        	</P>
	       	</TD>
	      </TR>
				<TR>
					<TD valign="bootom" colspan="2"><BR><BR><%tcWebApi_get("String_Entry","APWpsSettingNetWork","s")%></TD>
				</TR>
				<TR>
					<TD colspan="2">
					  <P align=center>
						  <INPUT id=WPSMode name=WPSMode onclick=wpsbtn(1); type=button value="<%tcWebApi_get("String_Entry","APWpsconnection","s")%>">
					  </P>
					 </TD>
				</TR>
				</TBODY>
			...
			<TR name="AP5GTitle" id="AP5GTitle" style='display:none'>
				<TD width=157 bgColor=#ef8218 height=30 >
				<P class=Item_L1><%tcWebApi_get("String_Entry","AP5GSettingTitle","s")%></P></TD>
				<TD width=7 bgColor=#ef8218>　</TD>         
				<TD width=474></TD>
			</TR>
			<TR name="AP5GConText" id="AP5GConText" style='display:none'>
				<TD vAlign=top width=157 bgColor=#e7e7e7 height=30>
				<P class=Item_L2></P></TD>
				<TD width=7 background=/img/panel3.gif>　</TD>
				<TD>
			...
			<TBODY>
				<TR>
      		<TD colSpan=2>
      			<INPUT onclick=ACManualSet() name=cb_ACMSEnbl <%if tcWebApi_get("apcli_entry1","Enable","h") = "1" then asp_Write("checked") end if%>  type=checkbox>使能5G AP配置
      	...
      	<TR>
      		<TD colspan="2">提示：请先在宽带配置页面选择传输模式为APClient-5G模式。</TD>
     		</TR>
        <TR>
				<TD>中继上行WLAN SSID：</TD>
				<TD>
					<LABEL> 
						<LABEL>
						<INPUT id=wlanACssidap class='input_s1' maxLength=63 size=15 name=wlanACssidap value="<%if tcWebApi_get("apcli_entry1","SSID","h") <> "N/A" then tcWebApi_get("apcli_entry1","SSID","s") else asp_write("CMCC-QLINK") end if%>"> 
						</LABEL>
					</LABEL>
				</TD>
			</TR>
				...
				<TD>
					<LABEL> 
						<SELECT id=wlACSecurMode onchange=ACauthModeChange() name="wlACSecurMode">
							<OPTION value="OPEN" <%if TCWebApi_get("apcli_entry1","AuthMode","h" ) = "OPEN" then asp_Write("selected") end if %>>OPEN</OPTION>
							<OPTION value="WEP_OPEN" <%if TCWebApi_get("apcli_entry1","AuthMode","h" ) = "WEP_OPEN" then asp_Write("selected") end if %>>WEP_OPEN</OPTION>
							<OPTION value="WEP_SHARED" <%if TCWebApi_get("apcli_entry1","AuthMode","h" ) = "WEP_SHARED" then asp_Write("selected") end if %>>WEP_SHARED</OPTION>
							<OPTION value="WPAPSK" <%if TCWebApi_get("apcli_entry1","AuthMode","h" ) = "WPAPSK" then asp_Write("selected") end if %>>WPA-PSK</OPTION>
							<OPTION value="WPA2PSK" <%if TCWebApi_get("apcli_entry1","AuthMode","h" ) = "WPA2PSK" then asp_Write("selected") end if %>>WPA2-PSK</OPTION>
						</SELECT>
					</LABEL>
				</TD>
			</TR>
			<TR id='ACwpapsw'>
				...
				<TD>
					<span id="ACinpsw">
					<INPUT style="FONT-FAMILY: '宋体'; width: 140px" type=password maxLength=65 id=wlACWpaPsk name=wlACWpaPsk value='<% if tcWebApi_get("apcli_entry1", "WPAPSK", "h") <> "N/A" then tcWebApi_get("apcli_entry1", "WPAPSK", "s") end if %>'>
					</span>
				</TD>
			</TR>
			<TR id='wlACWpaD'>
				...
				<TD>
					<LABEL> 
						<SELECT id=wlACWpa name="wlACWpa">
							<OPTION value="TKIP" <%if TCWebApi_get("apcli_entry1","EncrypType","h" ) = "TKIP" then asp_Write("selected") end if %>>TKIP</OPTION>
							<OPTION value="AES" <%if TCWebApi_get("apcli_entry1","EncrypType","h" ) = "AES" then asp_Write("selected") end if %>>AES</OPTION>
						</SELECT>
					</LABEL>
				</TD>
			</TR>
				...
				<TD>
					<LABEL> 
						<SELECT id=wlKeyBit name="wlACKeyBit">
							<OPTION value="128" <%if TCWebApi_get("apcli_entry1","KeyBit","h" ) = "128" then asp_Write("selected") end if %>>128-bit</OPTION>
							<OPTION value="64" <%if TCWebApi_get("apcli_entry1","KeyBit","h" ) = "64" then asp_Write("selected") end if %>>64-bit</OPTION>
						</SELECT>
					</LABEL>
				</TD>
			</TR>
				...
				<TD>
					<LABEL> 
						<SELECT id=wlACKeyIndex name="wlACKeyIndex">
							<OPTION value="1" <%if TCWebApi_get("apcli_entry1","DefaultKeyID","h" ) = "1" then asp_Write("selected") end if %>>1</OPTION>
							<OPTION value="2" <%if TCWebApi_get("apcli_entry1","DefaultKeyID","h" ) = "2" then asp_Write("selected") end if %>>2</OPTION>
							<OPTION value="3" <%if TCWebApi_get("apcli_entry1","DefaultKeyID","h" ) = "3" then asp_Write("selected") end if %>>3</OPTION>
							<OPTION value="4" <%if TCWebApi_get("apcli_entry1","DefaultKeyID","h" ) = "4" then asp_Write("selected") end if %>>4</OPTION>
						</SELECT>
					</LABEL>
				</TD>
			</TR>
			<TR id='wlACKey'>
				<TD class='w-130'><%tcWebApi_get("String_Entry","KeyText","s")%>	</TD>
				<TD><INPUT style="width: 140px" maxLength=26 name=wlACKeys value="<% if tcWebApi_get("apcli_entry1", "Key", "h") <> "N/A" then tcWebApi_get("apcli_entry", "Key", "s") end if %>"></TD>
			</TR>
			<TR id= "tr_btnSave2" name="tr_btnSave2">
        <TD colSpan=2>
        	<P align=center>
						<INPUT id=btnSave2 onclick=checkAPMode(1) name=btnSave2 value="<%tcWebApi_get("String_Entry","BtnSaveText","s")%>" type=button>
						<INPUT id=btnCancel2 onclick=onCancel() name=btnCancel2 value="<%tcWebApi_get("String_Entry","BtnCancelText","s")%>" type=button>
        	</P>
       	</TD>
      </TR>
			<TR>
			...
			<TR>
				<TD colspan="2">
				  <P align=center>
					  <INPUT id=acWPSMode name=acWPSMode onclick=wpsbtn(2); type=button value="<%tcWebApi_get("String_Entry","APWpsconnection","s")%>">
				  </P>
				 </TD>
			</TR>
			</TBODY>
