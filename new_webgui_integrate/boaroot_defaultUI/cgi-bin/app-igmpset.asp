...
%>
<HTML><HEAD><TITLE><%tcWebApi_get("String_Entry","IGMPMLD1Text","s")%></TITLE>
...
border=0>
        ...
        <TR>
          ...
          <TD width=434 bgColor=#427594 height=33>
            ...
            value="<%tcWebApi_get("String_Entry","ContentAPPText","s")%>-><%tcWebApi_get("String_Entry","ContentIGMPMLDText","s")%>" name=Selected_Menu> </FONT></B><SPAN 
...
<% end if %> 
	...
	function btnSubmit(iType)
	{
		if(0 == iType)
		{
			...
			if( true == setEBooValueCookie(document.SnoopForm) )
		...
		{	
			if(document.ConfigForm.chkIgmpProxy.checked)
			{
				...
				{
					alert("<%tcWebApi_get("String_Entry","IGMPMLD2Text","s")%>");
					return;
				}
			}
			else 
...
<% end if %>
        ...
        <TR>
		  ...
		  <FORM name="SnoopForm" action="/cgi-bin/app-igmpset.asp" method="post">                          
			<input type="hidden" id="Snoop_Flag" name="Snoop_Flag" value="0">			
			<input type="hidden" id="SnoopActive" name="SnoopActive" value="0">				  
                  <TABLE cellSpacing=3 cellPadding=3 border=0>
                    <TBODY>
                    <TR>
                      <TD>
                      <% if tcwebApi_get("WebCustom_Entry","isIPv6Supported","h") ="Yes" then %>
                      	<%tcWebApi_get("String_Entry","IGMPMLD3Text","s")%>
                      <% else %>
												<%tcWebApi_get("String_Entry","IGMPMLD4Text","s")%>
					       			<% end if %>
                      </TD>
                      <TD><INPUT type=checkbox id=chkIgmpSnp
                  name=chkIgmpSnp></TD></TR></TBODY></TABLE><BR><BR></FORM>
                  <CENTER><INPUT onclick='btnSubmit(0)' type=button value=" <%tcWebApi_get("String_Entry","IGMPMLD5Text","s")%> " id=applySnp name=applySnp> 
                  </CENTER>
                  <P></P></TD>
                <TD width=10>&nbsp;</TD></TR>
              <TR>
	...
	<input type="hidden" id="Proxy_Flag" name="Proxy_Flag" value="0">
	<input type="hidden" id="ProxyActive" name="ProxyActive" value="0">
            <TABLE cellSpacing=0 cellPadding=0 width="100%" border=0>
              <TBODY>
              <TR>
                <TD width=10>&nbsp;</TD>
                <TD>
                  ...
                  <TABLE cellSpacing=3 cellPadding=3 border=0>
                    ...
                    <TR>
                      <TD>
                      <% if tcwebApi_get("WebCustom_Entry","isIPv6Supported","h") ="Yes" then %>
                      	<%tcWebApi_get("String_Entry","IGMPMLD6Text","s")%>
					            <% else %>
												<%tcWebApi_get("String_Entry","IGMPMLD7Text","s")%>
					       			<% end if %>
                      </TD>
                  ...
                  border=1 name="IgmpProxyTab">
                    <TBODY>
                    <TR>
			<TD width=120><%tcWebApi_get("String_Entry","IGMPMLD8Text","s")%> </TD>
			...
			for (i = 0; i < nEntryNum; i++)
			{
				...
				//	igmpif_find = true;
                  ...
                  <CENTER><INPUT onclick='btnSubmit(1)' type=button value=" <%tcWebApi_get("String_Entry","IGMPMLD5Text","s")%> " id=applyProxy name=applyProxy> 
                  </CENTER>
                  <P></P></TD>
                <TD width=10>&nbsp;</TD></TR></TBODY></TABLE></TD></TR>
        <TR>
