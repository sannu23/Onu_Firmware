...
<title>IPv6 <%tcWebApi_get("String_Entry","StaticRouteText","s")%></title>
...
with ( document.forms[0] )
{
	for (i = 0; i < StaticRoute.length - 1; i++)
	{
		...
		{
			alert('<%tcWebApi_get("String_Entry","IPAddressIncorrect1Text","s")%>');
			return false;
		}
	}
	...
	if (DestIp == '')
	{
		alert('<%tcWebApi_get("String_Entry","IPAddressIncorrect2Text","s")%>');
		return false;
	}
	...
	{
		alert('<%tcWebApi_get("String_Entry","PrefixIncorrectText","s")%>');
		return false;
	}
	...
	if (gwStr.checked == true)
	{
		...
		{
			alert("<%tcWebApi_get("String_Entry","GatewayIncorrectText","s")%>");
			return false;
		}
	...
	{
		alert('<%tcWebApi_get("String_Entry","InterfaceIncorrectText","s")%>');
		return false;
	}
...
<tr>
<td width="157" bgcolor="#EF8218" height="30"><p class="Item_L1">IPv6 <%tcWebApi_get("String_Entry","StaticRouteText","s")%></p></td>
...
<td><p> 
                      ...
                      <input type="hidden" id="curSetIdx" name="curSetIdx" value="<% tcWebApi_get("Route6","add_num","s") %>">
                      <input type="hidden" id="user_def_num" name="user_def_num" value="<% tcWebApi_get("Route6","User_def_num","s") %>">
...
<% if tcwebApi_get("WebCustom_Entry","isNPTv6Supported","h" ) = "Yes" then %>
		if((vEntryName[i].indexOf('_INTERNET_')<0 && vEntryName[i].indexOf('_OTHER_')<0) 
<% else %>
		if((vEntryName[i].indexOf('_INTERNET_')<0) 
<% end if %>
		|| vEntryName[i].indexOf('_B_')>-1 || 'IPv4' == Wan_IPVERSION[i] )
				continue;
...
<tr>
<td width="100"><%tcWebApi_get("String_Entry","StaticRouteText","s")%></td>
<td width="309"><input type="checkbox" id='gEnable' name='gEnable' value="Yes"><%tcWebApi_get("String_Entry","EnableText","s")%></td>
</tr>
<tr>
<td width="100"><%tcWebApi_get("String_Entry","DestinationIPAddressText","s")%></td>
<td width="309"><input type='text' id='DestIPAddress' name='DestIPAddress' maxlength='64'></td>
</tr>
<tr>
<td><%tcWebApi_get("String_Entry","RadvdNamePrefixLenText","s")%></td>
<td><input type="text" id="PrefixLen" name="PrefixLen" maxlength="64" ><strong style="color:#FF0033">*</strong></td>
</tr>
</table>
<br>
<table>
<tr>
<td><input type="checkbox" id='gwStr' name='gwStr'>
&nbsp;<%tcWebApi_get("String_Entry","DefaultGatewayText","s")%>&nbsp;&nbsp;</td>
<td><input type='text' maxlength='64' id='GatewayIPAddress' name='GatewayIPAddress'></td>
</tr>
<tr>
<td>&nbsp;<%tcWebApi_get("String_Entry","UseInterfaceText","s")%></td>
